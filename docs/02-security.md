...
**Mitigación adoptada.** `webhook/WebhookUrlValidator.java` valida antes de cada envío (llamado desde `HttpWebhookSender.send`): si `requireHttps` es `true` (el valor de `application.yaml`, perfil no local), cualquier esquema `http` se rechaza sin excepción, esté o no el host en la allowlist; si `requireHttps` es `false` (perfil `local`, `application-local.yaml`), `http` se permite solo si el host está en `WEBHOOK_ALLOWLIST`. La resolución de DNS usa `InetAddress.getAllByName(host)` y toma la primera dirección; se rechazan direcciones site-local, loopback, link-local, "any local", multicast, y direcciones únicas locales IPv6 (`fc00::/7`) — **salvo que el host esté en la allowlist**, en cuyo caso ninguna de esas comprobaciones de rango se aplica (la allowlist es una excepción total para ese host, no solo para el esquema). `webhook/WebhookClientConfig.java` construye el `HttpClient` de Apache HttpClient 5 con `disableRedirectHandling()` y `disableAutomaticRetries()`, y con los timeouts de conexión/lectura de `WebhookProperties`; no configura un `DnsResolver` propio. Esto significa que la dirección que `WebhookUrlValidator` valida y la dirección a la que efectivamente se conecta `RestClient` no son necesariamente la misma: la conexión hace su propia resolución de DNS en el momento de conectar. **El cierre del DNS rebinding (conectar exactamente a la IP ya validada) no está implementado hoy**; queda como evolución. En AWS, security group de egreso solo hacia internet e IMDSv2 con límite de un salto; el NAT Gateway no es mitigación. Las suscripciones se cargan solo por migración (`V2__initial_subscriptions.sql`), sin CRUD, lo que reduce la superficie (RFC §16).

**Evolución.** Pinear la conexión HTTP a la dirección ya validada (resolutor de DNS propio en `WebhookClientConfig`) para cerrar el DNS rebinding. Con el CRUD de suscripciones esta validación será la primera línea de defensa; la verificación de URL con challenge no está en alcance (RFC §11, §16).
...
**Corrección sobre Swagger.** `springdoc-openapi-starter-webmvc-ui` es una dependencia normal del módulo `infrastructure` (`infrastructure/pom.xml`), sin condicionar a ningún perfil, y no hay configuración que la desactive fuera de `local`: Swagger UI y `/v3/api-docs` están disponibles en cualquier perfil, no solo en local. Lo que sí es cierto es que ninguna ruta de Swagger está en la lista `permitAll` de `SecurityConfig`, así que `anyRequest().authenticated()` la cubre: acceder a ella exige el mismo Bearer JWT que el resto de la API. No es pública, pero tampoco está deshabilitada fuera de local como se afirmaba antes.

**Evolución.** Restringir `springdoc-openapi-starter-webmvc-ui` a los perfiles de desarrollo (por ejemplo con `springdoc.api-docs.enabled` condicionado al perfil `local`). Imágenes distroless y multi-arquitectura quedan fuera del alcance (RFC §12).
...
| API7 SSRF | A10 | HTTPS salvo allowlist, rangos privados rechazados salvo allowlist, sin redirecciones, sin reintentos del cliente HTTP; **DNS no pineado** | `WebhookUrlValidator`, `WebhookClientConfig` | Pinear la IP validada para cerrar el rebinding; CRUD de suscripciones |
| API8 Misconfiguration | A05 | Errores con mensaje fijo, actuator acotado, sin root, BOM de Spring Boot; Swagger activo pero autenticado en todo perfil | `ApiExceptionHandler`, `application.yaml`, `docker/*.Dockerfile`, `pom.xml`, `SecurityConfig` | Restringir Swagger por perfil, distroless |
