...
[ -f deploy/local/keys/jwt-private.pem ] || { echo "run 'make keys' first" >&2; exit 1; }
...
signature=$(printf '%s.%s' "$header" "$payload" | openssl dgst -sha256 -sign deploy/local/keys/jwt-private.pem | b64url)
