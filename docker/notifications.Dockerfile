...
# resulting jar. The Maven project lives in services/notifications; the build context stays
...
COPY services/notifications/mvnw services/notifications/pom.xml ./
COPY services/notifications/.mvn .mvn
COPY services/notifications/domain/pom.xml domain/pom.xml
COPY services/notifications/application/pom.xml application/pom.xml
COPY services/notifications/infrastructure/pom.xml infrastructure/pom.xml
RUN chmod +x mvnw \
 && ./mvnw -q -B -pl infrastructure -am dependency:go-offline

# 2) Sources of the hexagon and its Spring Boot adapters. The simulator is a separate project.
COPY services/notifications/domain/src domain/src
COPY services/notifications/application/src application/src
COPY services/notifications/infrastructure/src infrastructure/src
...
RUN cp infrastructure/target/notifications.jar /workspace/app.jar \
 && cd /workspace \
